...
- (id)copyWithZone:(NSZone *)zone{
    ...
    NGObject *copy = [[[self class] alloc] init];
...
- (void)updateValue:(id)value forKey:(NSString*)key{
    ...
    if (property != NULL) {
        ...
        if ( [encodeType isSubclassOfClass:[NGObject class]]
            ...
            [(NGObject*)dyanaVal updateWithInfo:value];
        ...
        [CSDebugLog message:@"%@ property not found. So added to the FatBoy",key];
        [self setValue:value forKey:key];
    }
}
...
- (NSDictionary *)serializeIntoInfo{
    ...
    if (self.fatBoy.count > 0) {
        for (id key in [self.fatBoy allKeys]) {
...
- (id) serializeValue:(id)value forKey:(NSString*)key{
    ...
    id result = nil;
    if ([value isKindOfClass:[NGObject class]]) {
        result = [(NGObject*)value serializeIntoInfo];
    }
...
- (NSArray*) recursiveArraySerializer:(NSArray*)value{
    ...
    for (id item in value) {
        ...
        if ([item isKindOfClass:[NSArray class]]) {
        ...
        else if ([item isKindOfClass:[NGObject class]]) {
            ...
            [CSDebugLog message:@"Leef Item of encode type :: %@ has failed to parse.",NSStringFromClass([item class])];
...
- (NSDictionary*) recursiveDictionarySerializer:(NSDictionary*)value{
    ...
    for (id key in [value allKeys]) {
        ...
        if ([item isKindOfClass:[NSArray class]]) {
            ...
            [CSDebugLog message:@"Leef Key : %@ of encode type :: %@ has failed to parse.", key, NSStringFromClass([item class])];
